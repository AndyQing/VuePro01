...
下载项目，然后执行【npm install】，之后再执行【npm run dev】启动项目。
...
3. 发表评论的功能，直接引用现成的组件：
 ...
 3. 如何使用缩略图组件呢：
  ...
  3. 把复制的Ip地址，粘贴到`package.json`中：`"dev": "webpack-dev-server --open --port 3000 --hot --host 127.0.0.1"`， 将`127.0.0.1`修改为自己的WIFI的地址；
...
使用`webpack`打包项目，然后根目录下会自动生成dist文件夹
